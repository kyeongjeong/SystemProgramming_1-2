///////////////////////////////////////////////////////////////////////////////////////
// File Name    : 2021202078_spls_advanced.c                                         //
// Date         : 2023/04/05                                                         //
...
void listDirFiles(int a_hidden, int l_format, int S_size, int r_reverse, char* filename);
void printFileInfo(char* fileName, int l_format);
void sortByNameInAscii(char **fileList, int fileNum, int start, int r_reverse);
void sortByFileSize(char **fileList, char* dirPath, int fileNum, int start);
void printPermissions(mode_t mode);
void printType(struct stat fileStat);
void printAttributes(struct stat fileStat);
...
int main(int argc, char* argv[]) {
    ...
    if((a_hidden == 0) && (l_format == 0) && (S_size == 0) && (r_reverse == 0)) //옵션이 없는 경우
        optind = 0;
    ...
    if((argc-optind) == 1) { //인자가 없는 경우(옵션은 있을수도, 없을수도 있다)
        ...
        listDirFiles(a_hidden, l_format, S_size, r_reverse, currentPath); //현재 디렉토리 하위 파일 출력
    }
        ...
        char dirPath[MAX_LENGTH]; //출력할 절대경로
        ...
        if (S_size == 1) {
            ...
            sortByFileSize(temp, dirPath, argc, optind);

            for (int i = optind; i < argc; i++) { // 1. 존재하는 파일임(옵션 x) 2. 디렉토리가 아님 3. run파일이 아님
                
                if ((stat(argv[i], &st) != -1) && (!S_ISDIR(st.st_mode)) && (i != 0))
                    printFileInfo(temp[i], l_format);
            }
        }
            ...
            for (int i = optind; i < argc; i++) { // 1. 존재하는 파일임(옵션 x) 2. 디렉토리가 아님 3. run파일이 아님
                if ((stat(argv[i], &st) != -1) && (!S_ISDIR(st.st_mode)) && (i != 0))
                    printFileInfo(argv[i], l_format); // 파일정보 출력
            }
        }
        ...
        for(int i = optind; i < argc; i++) {
            if((stat(argv[i], &st) != -1) && S_ISDIR(st.st_mode)) {
                ...
                listDirFiles(a_hidden, l_format, S_size, r_reverse, argv[i]); //디렉토리 하위 파일정보 출력
...
void listDirFiles(int a_hidden, int l_format, int S_size, int r_reverse, char* filename) {
    ...
    while((dir = readdir(dirp)) != NULL) {
        ...
        strcat(accessFilename, dir->d_name);
        stat(accessFilename, &st);
    ...
    if (S_size == 1) {
        sortByFileSize(fileList, dirPath, fileNum, 0);
    }
    else
    ...
    for(int i = 0; i < fileNum; i++) {
        ...
        if(l_format == 1) { //옵션 -l이 포함된 경우
            ...
            strcat(accessPath, "/");
            strcat(accessPath, fileList[i]);

            stat(accessPath, &fileStat);
            printAttributes(fileStat);
        }
